...
function withFreshQuery(url, seed = "") {
  return `${url}${url.includes("?") ? "&" : "?"}_ts=${Date.now()}${seed ? `-${seed}` : ""}`;
}
...
async function fetchJSON(url, timeoutMs = 15000, attempts = 1) {
  let lastError;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      return JSON.parse(await fetchText(withFreshQuery(url, attempt), timeoutMs));
    } catch (error) {
      lastError = error;
...
async function buildNews() {
  ...
  try {
    ...
    const dataDate = json.data?.date || ymd();
    const stale = !sameYmd(dataDate);
    return {
      date: ymd(),
      dataDate,
      generatedAt,
      sourceCreated: json.data?.created || "",
      sourceCreatedAt: json.data?.created_at || null,
      sourceUpdated: json.data?.updated || "",
      sourceUpdatedAt: json.data?.updated_at || null,
      apiUpdated: json.data?.api_updated || "",
      apiUpdatedAt: json.data?.api_updated_at || null,
      source: "60秒读懂世界",
      stale,
      message: stale ? `新闻源尚未更新到 ${ymd()}，未展示旧新闻。上一期为 ${dataDate}。` : "",
      news: stale ? [] : news,
      financeSignals: stale ? [] : buildFinanceSignals(news),
    };
  } catch (error) {
    return {
      date: ymd(),
...
async function fetchIndices() {
  ...
  const json = await fetchJSON(url, 20000, 4);
...
async function fetchMarketBreadth() {
  ...
  const pageSize = 100;
  const base =
    "https://push2delay.eastmoney.com/api/qt/clist/get?po=1&ut=bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&fid=f3&fs=" +
    encodeURIComponent(marketFs) +
    "&fields=f3,f12,f14,f100,f62&np=1&pn=";
  const first = await fetchJSON(`${base}1&pz=${pageSize}`, 20000, 4);
  ...
  let approxLimitUp = 0;
  let netInflow = 0;
  let netInflowCount = 0;
  const errors = [];

  function consume(pageJson) {
    for (const x of normalizeDiff(pageJson.data?.diff)) {
      const money = Number(x.f62);
  ...
  for (let page = 2; page <= pages; page += 1) {
    try {
      consume(await fetchJSON(`${base}${page}&pz=${pageSize}`, 20000, 4));
    } catch (error) {
      errors.push(`page ${page}: ${error.message || error}`);
    }
    await sleep(80);
  }
  ...
  return {
    ...
    netInflow,
    netInflowCount,
    netInflowComplete: netInflowCount === counted,
    scope: "沪深京A股",
    provider: "东方财富延迟行情",
    complete: errors.length === 0 && counted === total,
    error: errors[0] || (counted !== total ? `breadth incomplete ${counted}/${total}` : ""),
  };
}

async function buildMarket() {
  const generatedAt = new Date().toISOString();
  const indicesResult = await fetchIndices().then(
    (value) => ({ status: "fulfilled", value }),
    (reason) => ({ status: "rejected", reason }),
  );
  const limitResult = await fetchLimitUpPool().then(
    (value) => ({ status: "fulfilled", value }),
    (reason) => ({ status: "rejected", reason }),
  );
  const breadthResult = await fetchMarketBreadth().then(
    (value) => ({ status: "fulfilled", value }),
    (reason) => ({ status: "rejected", reason }),
  );
  ...
  const breadth = breadthResult.status === "fulfilled" ? breadthResult.value : { total: 0, up: 0, down: 0, flat: 0, approxLimitUp: 0, netInflow: null, netInflowCount: 0, netInflowComplete: false };
  ...
  return {
    ...
    dataDate: limit.qdate || ymd(),
    dataScope: breadth.scope || "沪深京A股",
    breadthProvider: breadth.provider || "东方财富延迟行情",
    breadthComplete: breadth.complete === true,
    breadthCounted: breadth.counted || 0,
    unpriced: breadth.unpriced || 0,
    indices,
    total: breadth.total,
    up: breadth.up,
    down: breadth.down,
    flat: breadth.flat,
    netInflow: breadth.netInflow,
    netInflowCount: breadth.netInflowCount || 0,
    netInflowComplete: breadth.netInflowComplete === true,
    ...
    errors: [
      ...
      breadthResult.status === "fulfilled" && breadthResult.value.error ? `breadth: ${breadthResult.value.error}` : "",
    ].filter(Boolean),
  };
}
